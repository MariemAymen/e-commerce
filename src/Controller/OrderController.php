...
class OrderController extends AbstractController
{
    ...
    public function index(SessionInterface $session): Response
    {
        ...
        // dd($order);
        $cartItems = $session->get('cart', []); // return empty
        ...
        foreach ($session->get('cart', []) as $cartItem) {
            ...
            $orderDetail->setCretedAt(new \DateTimeImmutable());
            $orderDetail->setUpdatedAt(new \DateTimeImmutable());

            $total += $cartItem['quantity'] * $cartItem['product']->getPrix(); // le prix totale de commande
